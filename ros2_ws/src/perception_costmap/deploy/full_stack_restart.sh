#!/bin/bash
# Rebuild the whole percept tmux session from scratch: sensors (TF+lidar),
...
tmux kill-session -t percept 2>/dev/null || true
...
# detaches — learned the hard way.)
systemd-reset-failed --user percept-tmux 2>/dev/null || true
systemd-run --user --scope --collect --unit percept-tmux \
  tmux new-session -d -s percept -n sensors \
...
tmux new-window -t percept -n costmap \
  "bash -c \"source /opt/ros/humble/setup.bash && source /home/dinosaur/carla-nav2-avl/ros2_ws/install/setup.bash && $E && ros2 launch perception_costmap perception.launch.py config:=/home/dinosaur/perception_3cam.yaml 2>&1 | tee /tmp/costmap.log; exec bash\""
sleep 15

echo "[6/6] viz node + streaming servers..."
tmux new-window -t percept -n viz \
  "bash -c \"source /opt/ros/humble/setup.bash && $E && python3 /home/dinosaur/viz_node.py 2>&1 | tee /tmp/viz_node.log; exec bash\""
tmux new-window -t percept -n wvs \
